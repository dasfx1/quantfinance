...
def _load_csv_records(
    ...
    with source.path.open("r", newline="") as f:
...
def load_price_bars(
    ...
    *,
    ...
    data_obj: Union["pd.DataFrame", CSVDataSource, None] = None

    if prefer_local:
        try:
            data_obj = CSVDataSource(
                path=_fallback_csv_path(ticker),
                fromdate=_parse_date(start),
                todate=_parse_date(end),
            )
    ...
    if data_obj is None:
        data_obj = get_data(ticker, start, end)
    ...
    if is_dataframe(data_obj):
        df = data_obj.loc[start:end][["Open", "High", "Low", "Close", "Volume"]]
        ...
        assert isinstance(data_obj, CSVDataSource)
        records = _load_csv_records(data_obj, start_dt, end_dt)

    if not records and not prefer_local:
        fallback_source = CSVDataSource(
            path=_fallback_csv_path(ticker),
            fromdate=start_dt,
            todate=end_dt,
        )
        records = _load_csv_records(fallback_source, start_dt, end_dt)
